...
export const Sidebar: React.FC<SidebarProps> = ({ currentPath, onNavigate, isMobileOpen = false, onCloseMobile }) => {
  const { role } = useAuth();

  const memberNav = [
    { label: 'Dashboard', path: '/member/dashboard', icon: <LayoutDashboard size={18} /> },
    { label: 'Library Catalog', path: '/member/catalog', icon: <Book size={18} /> },
  ...
  const librarianNav = [
    ...
    { label: 'Borrowing Records', path: '/librarian/records', icon: <History size={18} /> },
